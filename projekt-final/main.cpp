 ...
 *
 * Program wczytuje opis układu bramek z pliku podanego przez użytkownika oraz zestawy wejść z plików tekstowych,
 * a następnie symuluje propagację sygnałów przez układ z uwzględnieniem
 * czasów propagacji poszczególnych bramek.
 * Obecna wersja programu jest modelem iteracyjnym.
 */
  ...
  * Pierwsza linia zawiera nagłówki sygnałów wejściowych a i b.
...
int main() {
    string nazwa_pliku = "";
    cout << "Podaj nazwe pliku tekstowego(.txt)(bez rozszerzenia), ktory zawiera poprawnie opisany uklad kombinacyjny" << endl;
    cin >> nazwa_pliku;
    fstream opis(nazwa_pliku+".txt", ios::in);
    if (!opis.good()) { cout << "Nie mozna otworzyc podanego pliku"; return 1; } 
